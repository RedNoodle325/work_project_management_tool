...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="x-shell">
    ...
    <aside className={`x-sidebar ${open ? 'open' : ''}`}>
      ...
      <footer>{user ? <><div className="x-user-mark">{(user.name || user.email || 'Z').charAt(0).toUpperCase()}</div><div><strong>{user.name || 'Workspace owner'}</strong><span>{user.email}</span></div><button onClick={logout} title="Sign out"><LogOut size={17} /></button></> : <div><Link href="/login?mode=scheduler" className="x-public-sign-in"><LogIn size={17} /><span>Scheduler sign in</span></Link><Link href="/login" style={{ display: 'block', marginTop: 9, paddingLeft: 4, color: 'var(--text3)', fontSize: 11, opacity: 0.75 }}>Personal workspace login</Link></div>}</footer>
    </aside>
