...
export function Layout({ children }: { children: ReactNode }) {
  ...
  return <div className="x-shell">
    ...
    <aside className={`x-sidebar ${open ? 'open' : ''}`}>
      ...
      <nav><span>Workspace</span>{nav.map(item => { const Icon = item.icon; const active = item.href === '/' ? path === '/' : path.startsWith(item.href); return <Link key={item.href} href={item.href} className={active ? 'active' : ''} onClick={() => setOpen(false)}><Icon size={18} /><span>{item.label}</span>{active && <i />}</Link> })}{user && <Link href="/todos" className={path.startsWith('/todos') ? 'active' : ''} onClick={() => setOpen(false)}><ListChecks size={18} /><span>My To-Do List</span>{path.startsWith('/todos') && <i />}</Link>}</nav>
