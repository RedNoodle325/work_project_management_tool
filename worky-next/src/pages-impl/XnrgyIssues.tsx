...
export function XnrgyIssues() {
  ...
  useEffect(() => {
    Promise.all([V2.issues.list(), V2.hierarchy.list(), API.projectJobs.list()]).then(([issueRows, hierarchy, jobData]) => {
      setIssues(issueRows)
      setJobs(jobData.jobs.filter(job => job.site_id))
  ...
  const filtered = useMemo(() => {
    ...
    return issues.filter(issue => [issue.issue_number, issue.description, issue.equipment_name, issue.serial_number, issue.job_number, issue.project_name, issue.project_code]
  ...
  return <div className="x-page x-issues-page">
    ...
    {showImport && <ImportIssues jobs={jobs} initialSiteId={siteId} close={() => setShowImport(false)} imported={(selectedSiteId) => { setShowImport(false); changeSite(selectedSiteId) }} />}
    {showCreate && <CreateIssue jobs={jobs} initialSiteId={siteId} close={() => setShowCreate(false)} saved={(selectedSiteId) => { setShowCreate(false); changeSite(selectedSiteId) }} />}
    {editIssue && <EditIssue issue={editIssue} jobs={jobs} close={() => setEditIssue(null)} saved={(updated) => { setIssues(current => siteId && updated.site_id !== siteId ? current.filter(issue => issue.id !== updated.id) : current.map(issue => issue.id === updated.id ? updated : issue)); setEditIssue(null) }} />}
...
function EditIssue({ issue, jobs, close, saved }: { issue: LeanIssueV2; jobs: ProjectJob[]; close: () => void; saved: (issue: LeanIssueV2) => void }) {
  const [form, setForm] = useState({
    project_job_id: issue.project_job_id || '',
  ...
  return <div className="x-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><form className="x-modal" onSubmit={submit}>
    ...
    <p className="x-import-intro">Every issue belongs to a job. Clear the issue number to assign the next number for the selected job.</p>
    <JobSelect jobs={jobs} value={form.project_job_id} change={set('project_job_id')} />
    <label className="x-field"><span>Issue number</span><input value={form.issue_number} onChange={event => set('issue_number')(event.target.value)} placeholder="Auto-assigned from job" /></label>
...
function CreateIssue({ jobs, initialSiteId, close, saved }: { jobs: ProjectJob[]; initialSiteId: string; close: () => void; saved: (siteId: string) => void }) {
  const eligibleJobs = initialSiteId ? jobs.filter(job => job.site_id === initialSiteId) : jobs
  const [form, setForm] = useState({ project_job_id: eligibleJobs.length === 1 ? eligibleJobs[0].id : '', issue_number: '', description: '', equipment_name: '', serial_number: '', status: 'open', priority: 'normal', source_url: '' })
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const set = (field: keyof typeof form) => (value: string) => setForm(current => ({ ...current, [field]: value }))
  async function submit(event: FormEvent) {
    event.preventDefault(); setSaving(true); setError('')
    try { await V2.issues.create(form); saved(jobs.find(job => job.id === form.project_job_id)?.site_id || '') }
    catch (error) { setError((error as Error).message) }
    finally { setSaving(false) }
  }
  return <div className="x-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><form className="x-modal" onSubmit={submit}>
    ...
    <p className="x-import-intro">Add an issue to a specific job. Leave the issue number blank to assign the next number for that job.</p>
    <JobSelect jobs={eligibleJobs} value={form.project_job_id} change={set('project_job_id')} />
    <label className="x-field"><span>Issue number</span><input value={form.issue_number} onChange={event => set('issue_number')(event.target.value)} placeholder="Auto-assigned from job" /></label>
    <label className="x-field"><span>Description</span><textarea value={form.description} onChange={event => set('description')(event.target.value)} rows={3} placeholder="Describe the issue" /></label>
    <div className="x-form-row"><label className="x-field"><span>Equipment / asset tag</span><input value={form.equipment_name} onChange={event => set('equipment_name')(event.target.value)} /></label><label className="x-field"><span>Serial #</span><input value={form.serial_number} onChange={event => set('serial_number')(event.target.value)} /></label></div>
    <div className="x-form-row"><label className="x-field"><span>Status</span><select value={form.status} onChange={event => set('status')(event.target.value)}><option value="open">Open</option><option value="in_progress">In progress</option><option value="scheduled">Scheduled</option><option value="waiting_parts">Waiting parts</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select></label><label className="x-field"><span>Priority</span><select value={form.priority} onChange={event => set('priority')(event.target.value)}><option value="low">Low</option><option value="normal">Normal</option><option value="high">High</option><option value="critical">Critical</option></select></label></div>
...
function ImportIssues({ jobs, initialSiteId, close, imported }: { jobs: ProjectJob[]; initialSiteId: string; close: () => void; imported: (siteId: string) => void }) {
  const picker = useRef<HTMLInputElement>(null)
  const eligibleJobs = initialSiteId ? jobs.filter(job => job.site_id === initialSiteId) : jobs
  const [projectJobId, setProjectJobId] = useState(eligibleJobs.length === 1 ? eligibleJobs[0].id : '')
  ...
  async function submit(event: FormEvent) {
    ...
    if (!projectJobId || !file) return
    setSaving(true); setError(''); setNote('')
    try {
      const result = await V2.issues.importCxAlloy(projectJobId, file)
      ...
      window.setTimeout(() => imported(jobs.find(job => job.id === projectJobId)?.site_id || ''), 900)
  ...
  return <div className="x-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><form className="x-modal" onSubmit={submit}>
    ...
    <JobSelect jobs={eligibleJobs} value={projectJobId} change={setProjectJobId} />
    ...
    <footer><button type="button" onClick={close}>Cancel</button><button className="primary" disabled={saving || !projectJobId || !file}>{saving ? 'Importing…' : 'Import issues'}</button></footer>
  </form></div>
}

function JobSelect({ jobs, value, change }: { jobs: ProjectJob[]; value: string; change: (value: string) => void }) {
  return <label className="x-field"><span>Job / project number</span><select value={value} onChange={event => change(event.target.value)} required><option value="">Choose a job</option>{jobs.map(job => <option value={job.id} key={job.id}>{job.job_number} · {job.name}{job.site_name ? ` — ${job.site_name}` : ''}</option>)}</select></label>
}
