...
export function XnrgySites() {
  ...
  useEffect(() => { load() }, [])
  const filtered = useMemo(() => customers.map(customer => ({
    ...customer,
    sites: (customer.sites || []).filter(site => `${customer.name} ${site.name} ${site.site_code || ''}`.toLowerCase().includes(query.toLowerCase())),
    locations: customer.locations.map(location => ({ ...location, sites: (location.sites || []).filter(site => `${customer.name} ${location.campus_code} ${location.city} ${site.name}`.toLowerCase().includes(query.toLowerCase())) })).filter(location => !query || (location.sites || []).length),
  })).filter(customer => !query || customer.locations.length || customer.sites.length), [customers, query])
  return <div className="x-page">
    ...
    <div className="x-customer-list">{filtered.map(customer => <section key={customer.id} className="x-customer"><header><div className="x-customer-icon"><Building2 size={20} /></div><div><span>Customer</span><h2>{customer.name}</h2></div><b>{customer.locations.reduce((total, location) => total + (location.sites?.length || 0), 0) + (customer.sites?.length || 0)} sites</b></header><div className="x-campus-list">{customer.sites?.length > 0 && <div className="x-campus"><div className="x-campus-head"><MapPin size={16} /><div><strong>Standalone sites</strong><span>No campus</span></div></div><div className="x-directory-sites">{customer.sites.map(site => <SiteLink key={site.id} site={site} />)}</div></div>}{customer.locations.map(location => <div className="x-campus" key={location.id}><div className="x-campus-head"><MapPin size={16} /><div><strong>{location.campus_code}</strong><span>{location.city}, {location.state}</span></div></div><div className="x-directory-sites">{location.sites?.map(site => <SiteLink key={site.id} site={site} />)}</div></div>)}</div></section>)}{!filtered.length && <div className="x-empty">No sites match that search.</div>}</div>
...
function CreateHierarchy({ kind, customers, close, saved }: { kind: NewKind; customers: HierarchyCustomerV2[]; close: () => void; saved: () => void }) {
  ...
  return <div className="x-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><form className="x-modal" onSubmit={submit}><header><div><span className="x-kicker">Add to directory</span><h2>New {kind === 'location' ? 'campus' : kind}</h2></div><button type="button" onClick={close}><X size={18} /></button></header>
    ...
    {kind === 'site' && <><Select label="Customer" value={form.customer_id} change={value => setForm(current => ({ ...current, customer_id: value, location_id: '' }))} options={customers.map(customer => [customer.id, customer.name])} /><Select label="Campus (optional)" value={form.location_id} change={set('location_id')} options={customerLocations.map(location => [location.id, `${location.campus_code} · ${location.city}, ${location.state}`])} required={false} emptyLabel="No campus — standalone site" /><Field label="Site name" value={form.name} change={set('name')} required placeholder="e.g. TOR1A" /><div className="x-form-row"><Field label="Site code" value={form.site_code} change={set('site_code')} /><Field label="Building" value={form.building} change={set('building')} /></div>{standalone && <><div className="x-form-row"><Field label="City" value={form.city} change={set('city')} required /><Field label="State / province" value={form.state} change={set('state')} required /></div><Field label="Street address" value={form.address} change={set('address')} /></>}</>}
...
function SiteLink({ site }: { site: HierarchyCustomerV2['sites'][number] }) { return <Link href={`/sites/${site.id}`}><div className={`x-status-dot is-${site.status}`} /><div><strong>{site.name}</strong><span>{site.lifecycle_phase} · {site.unit_count} units · {site.open_issue_count} open issues</span></div><ArrowRight size={16} /></Link> }
