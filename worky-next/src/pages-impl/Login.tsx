...
export function Login() {
  ...
  return <main className="x-login"><section className="x-login-brand"><img src="/brand/xnrgy-mark.svg" alt="XNRGY" /><div><span>Shared workspace</span><h1>The details are the work.</h1><p>Visitors can browse the tracker. Sign in with the owner account or a scheduler-only account to make authorized changes.</p></div><small>Role-based editing</small></section><section className="x-login-form"><form onSubmit={submit}><span className="x-kicker">{setupRequired ? 'First-time setup' : 'Welcome back'}</span><h2>{setupRequired ? 'Create your owner account' : 'Sign in to edit'}</h2><p>{setupRequired ? 'The first account owns the workspace and can create scheduler-only access.' : 'Scheduler accounts open directly to the employee scheduler.'}</p>{setupRequired && <label><span>Your name</span><div><input required autoFocus value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="Your name" /></div></label>}<label><span>Email address</span><div><Mail size={16} /><input type="email" required autoFocus={!setupRequired} value={email} onChange={event => setEmail(event.target.value)} placeholder="scheduler@zaktrack.pm" /></div></label><label><span>Password</span><div><LockKeyhole size={16} /><input type="password" required minLength={8} value={password} onChange={event => setPassword(event.target.value)} placeholder="At least 8 characters" /></div></label>{error && <div className="x-error">{error}</div>}<button disabled={loading}>{loading ? (setupRequired ? 'Creating account…' : 'Signing in…') : (setupRequired ? 'Create owner account' : 'Sign in')}<ArrowRight size={16} /></button></form></section></main>
}
