...
export function Jobs() {
  ...
  const [data, setData] = useState<ProjectJobsResponse>({ jobs: [], project_managers: [] })
  ...
  const visible = useMemo(() => {
    ...
    return data.jobs.filter(job => {
      ...
      return !text || [job.job_number, job.project_code, job.name, job.representative_code, job.assigned_pm_name, job.assigned_pm_email]
  ...
  return <main className="x-page x-jobs-page">
    <header className="x-directory-head">
      <div><span className="x-kicker">Project directory</span><h1>Jobs</h1><p>{data.jobs.length} released jobs · {mineCount} assigned to {data.current_user?.display_name || 'you'}</p></div>
    ...
    <section className="x-jobs-toolbar" aria-label="Job filters">
      ...
      <label className="x-directory-search"><Search size={17} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search job number, project, rep, or PM…" /></label>
    </section>
    ...
    {error ? <div className="x-load-panel"><strong>Jobs are temporarily unavailable</strong><p>{error}</p><button className="primary" onClick={load}>Try again</button></div> : loading ? <div className="x-jobs-loading">Loading jobs…</div> : <section className="x-jobs-list" aria-live="polite">
      <div className="x-jobs-head"><span>Job</span><span>Project</span><span>Representative</span><span>Project manager</span></div>
      {visible.map(job => {
