...
function SiteCard({ site, compact }: { site: DashboardData['sites'][number]; compact?: boolean }) { return <Link href={`/sites/${site.id}`} className={`x-site-card ${compact ? 'is-compact' : ''}`}><div className={`x-status-dot is-${site.status}`} /><div><strong>{site.name}</strong><span>{[site.customer_name, site.campus_code].filter(Boolean).join(' · ')}</span>{!compact && <small>{site.city}, {site.state}</small>}</div><ArrowRight size={15} /></Link> }
