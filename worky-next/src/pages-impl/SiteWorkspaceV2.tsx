...
function CreateModal({ kind, data, siteId, onClose, onSaved }: { kind: CreateKind; data: SiteWorkspaceV2; siteId: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <Modal title={`Add ${kind.replace('_',' ')}`} onClose={onClose} maxWidth={560}><form className="ops-form" onSubmit={submit}>
    ...
    {kind === 'issue' && <><SelectField label="Job / project number" value={form.project_job_id} onChange={set('project_job_id')} options={data.released_jobs.map(job => [job.id, `${job.job_number} · ${job.name}`])} /><SelectField label="ASR" value={form.asr_id} onChange={set('asr_id')} options={asrOptions} /><SelectField label="Unit" value={form.unit_id} onChange={set('unit_id')} options={[['','Site-wide'], ...data.units.map(u => [u.id,u.tag])]} /><TextField label="Issue" value={form.title} onChange={set('title')} required /><TextArea label="Description" value={form.description} onChange={set('description')} /><SelectField label="Priority" value={form.priority || 'normal'} onChange={set('priority')} options={['low','normal','high','critical'].map(v => [v,v])} /></>}
