...
export function XnrgySiteWorkspace() {
  ...
  return <div className="x-site-page">
    <header className="x-site-header">
      ...
      <div className="x-site-heading"><div><div className="x-breadcrumb">{site.customer_name}{site.campus_code && <> <span>/</span> {site.campus_code}</>}{site.project_number && <> <span>/</span> Job {site.project_number}</>}</div><h1>{site.name}</h1><p><MapPin size={14} /> {site.address && `${site.address} · `}{site.city}, {site.state}{site.postal_code && ` ${site.postal_code}`} · {site.lifecycle_phase}{site.representative_name && ` · Rep ${site.representative_name}`}</p></div><div><div className={`x-health-pill is-${site.status}`}><i />{site.status}</div><div className="x-head-actions" style={{ marginTop: 10 }}><button onClick={copyAddress} disabled={!mailingAddress}>{copied ? <Check size={14} /> : <Copy size={14} />} {copied ? 'Copied' : 'Copy address'}</button><button onClick={() => setEditing(true)}><Pencil size={14} /> Edit</button><button onClick={deleteSite}><Trash2 size={14} /> Delete</button></div></div></div>
...
function SiteSchedule({ siteId }: { siteId: string }) {
  ...
  const [error, setError] = useState('')
  const load = () => V2.sites.schedule.list(siteId).then(data => { setEvents(data.events); setChanges(data.changes) }).catch(error => setError(error.message))
...
function SiteEditor({ data, close, saved }: { data: SiteWorkspaceV2; close: () => void; saved: () => void }) {
  const site = data.site
  const [form, setForm] = useState({ name: site.name, project_number: site.project_number || '', representative_name: site.representative_name || '', site_code: site.site_code || '', building: site.building || '', city: site.city || '', state: site.state || '', address: site.address || '', postal_code: site.postal_code || '', status: site.status, lifecycle_phase: site.lifecycle_phase, notes: site.notes || '' })
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  ...
  return <div className="x-modal-backdrop" onMouseDown={event => event.target === event.currentTarget && close()}><form className="x-modal" onSubmit={submit}><header><div><span className="x-kicker">Site settings</span><h2>Edit {site.name}</h2></div><button type="button" onClick={close}><X size={18} /></button></header><label className="x-field"><span>Site name</span><input value={form.name} onChange={set('name')} required /></label><div className="x-form-row"><label className="x-field"><span>Job number</span><input value={form.project_number} onChange={set('project_number')} /></label><label className="x-field"><span>Representative</span><input value={form.representative_name} onChange={set('representative_name')} /></label></div><div className="x-form-row"><label className="x-field"><span>Site code</span><input value={form.site_code} onChange={set('site_code')} /></label><label className="x-field"><span>Building</span><input value={form.building} onChange={set('building')} /></label></div>{!site.location_id && <><div className="x-form-row"><label className="x-field"><span>City</span><input value={form.city} onChange={set('city')} required /></label><label className="x-field"><span>State / province</span><input value={form.state} onChange={set('state')} required /></label></div><div className="x-form-row"><label className="x-field"><span>Street address</span><input value={form.address} onChange={set('address')} /></label><label className="x-field"><span>ZIP / postal code</span><input value={form.postal_code} onChange={set('postal_code')} /></label></div></>}<div className="x-form-row"><label className="x-field"><span>Status</span><select value={form.status} onChange={set('status')}>{['planning','active','attention','critical','offline','complete','inactive'].map(value => <option key={value}>{value}</option>)}</select></label><label className="x-field"><span>Lifecycle</span><select value={form.lifecycle_phase} onChange={set('lifecycle_phase')}>{['planning','construction','commissioning','warranty','service','closed'].map(value => <option key={value}>{value}</option>)}</select></label></div><label className="x-field"><span>Notes</span><textarea value={form.notes} onChange={set('notes')} rows={3} /></label>{error && <p className="x-error">{error}</p>}<footer><button type="button" onClick={close}>Cancel</button><button className="primary" disabled={saving}>{saving ? 'Saving…' : 'Save site'}</button></footer></form></div>
}
