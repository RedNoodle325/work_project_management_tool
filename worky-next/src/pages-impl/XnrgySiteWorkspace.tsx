...
function Issues({ data }: { data: SiteWorkspaceV2 }) { return <Section title="Issues & ASRs" intro="Imported CxAlloy issues can stand alone or be connected to an ASR."><div className="x-table"><div className="x-table-head"><span>Issue</span><span>Unit</span><span>ASR</span><span>Priority</span><span>Status</span></div>{data.issues.map(issue => <div className="x-table-row" key={issue.id}><strong>{issue.title}</strong><span>{issue.unit_tag || 'Site-wide'}</span><code>{issue.asr_number || '—'}</code><em className={`is-${issue.priority}`}>{issue.priority}</em><span>{issue.status}</span></div>)}</div>{!data.issues.length && <Empty text="No issues have been recorded for this site." />}</Section> }
