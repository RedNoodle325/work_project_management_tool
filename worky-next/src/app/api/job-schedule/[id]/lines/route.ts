...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await sql.begin(async tx => {
    ...
    const [next] = await tx`select coalesce(max(line_number), 0) + 1 as value from public.job_schedule_lines where job_id = ${id}`
    const [line] = await tx`
    ...
    for (const technicianId of technicianIds) await tx`insert into public.job_schedule_line_techs (line_id, technician_id) values (${line.id}, ${technicianId}) on conflict do nothing`
    return line
